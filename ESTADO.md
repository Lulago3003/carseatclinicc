...
- [x] **Derivación directa a WhatsApp (29 jul 2026)**: el saludo es corto y las preguntas simples se responden dentro del chat. Cotización, alquiler, instalación, revisión, envíos, pagos, choques y casos de seguridad muestran **un único botón contextual** que abre WhatsApp con el tema, la pregunta y los datos ya compartidos escritos. Se quitó el botón duplicado, se limita cada respuesta visible a una burbuja corta y la IA remota queda apagada solo para el chat público (`respuestasRemotasEnChat:false`) para que no vuelva a reemplazarla por un texto enorme. El resumen de conversaciones del CRM puede seguir usando la Edge Function.
- [x] Acciones dentro del chat: reservar horario o fechas desde la web como opción secundaria, guardar caso/consulta en CRM y continuar por WhatsApp cuando hace falta asesor. El formulario final crea la reserva real, sin intentar cambiar una solicitud pública desde el navegador.
...
- [x] Pruebas automáticas del asistente: `scripts/chat-assistant-check.mjs` y `scripts/chat-widget-check.mjs`; el chequeo general `scripts/site-experience-check.mjs` ya valida que el chat inteligente esté conectado.
