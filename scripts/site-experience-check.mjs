...
const checks = [
  ...
  ["fechas de alquiler publicadas", files.alquiler.includes('id="rentalCalendar"') && files.alquiler.includes('id="rentalAvailabilityId"') && files.js.includes("function loadRentalAvailability")],
  ["rental day calculation", files.js.includes("function rentalDays") && files.js.includes("function updateRentalPanel")],
  ["alquiler valida la disponibilidad elegida", files.js.includes("function selectedRentalAvailabilityMatches") && files.js.includes('status: isRental ? "esperando_reserva" : "nuevo"')],
  ["web consulta disponibilidad pública", files.db.includes("function getRentalAvailability") && files.db.includes('from("rental_availability")')],
